...
class ObjectName extends Phaser.Scene {
    ...
    create() {
        ...
        // objects
            this.object = new Object(this, x, y, 'image');

            this.objectWithAnim = new Object(this, x, y, 'spritesheet', frame);

            this.add.image(x, y, 'image');    // just some image
            ...
            let roll = Math.floor(Math.random() * bank.length);     // random index from bank
                
            if (queue.length <= 1) {
                queue.push(bank[roll]);     // push that random one
                bank.splice(roll, 1);       // remove from bank
            }

            queue[1].inFront = queue[0];    // access attributes

        // player

            this.player = new Player(this, game.config.width/2, game.config.height - borderUISize - borderPadding, 'playerSprite').setOrigin(0.5, 0);;
